...
namespace datetime {
	namespace {
		...
		std::unordered_map<std::string, std::pair<std::string, const time_zone*>> _cached_zones;
	...
	DateTime::DateTime(const DateTime& other) {
		_time_point = new system_time_point(*(other._time_point));
		_h24 = other._h24;
		_tz = other._tz;
		_tz_name = other._tz_name;
		h12format = other.h12format;
		h24format = other.h24format;
	}

	void DateTime::_constructor_proxy(system_time_point* tp, bool h24, string tz) {
		_time_point = tp;
		_h24 = h24;
		if (tz != "") {
			set_timezone(tz);
	...
	void DateTime::_no_h24_proxy(system_time_point* tp, string tz) {
		_constructor_proxy(tp, true, tz);
	}

	DateTime::DateTime(bool h24, string time_zone) {
		_constructor_proxy(new system_time_point(), h24, time_zone);
	}

	DateTime::DateTime(const sys_days& timeval, bool h24, string time_zone) {
		_constructor_proxy(new system_time_point(timeval), h24, time_zone);
	}

	DateTime::DateTime(const system_duration& timeval, bool h24, string time_zone) {
		_constructor_proxy(new system_time_point(timeval), h24, time_zone);
	}

	DateTime::DateTime(const system_time_point& timeval, bool h24, string time_zone) {
		_constructor_proxy(new system_time_point(timeval), h24, time_zone);
	}

	DateTime::DateTime(const time_point_seconds& timeval, bool h24, string time_zone) {
		_constructor_proxy(new system_time_point(timeval), h24, time_zone);
	}

	DateTime::DateTime(string time_zone) {
		_no_h24_proxy(new system_time_point(), time_zone);
	}

	DateTime::DateTime(const sys_days& timeval, string time_zone) {
		_no_h24_proxy(new system_time_point(timeval), time_zone);
	}

	DateTime::DateTime(const system_duration& timeval, string time_zone) {
		_no_h24_proxy(new system_time_point(timeval), time_zone);
	}

	DateTime::DateTime(const system_time_point& timeval, string time_zone) {
		_no_h24_proxy(new system_time_point(timeval), time_zone);
	}

	DateTime::DateTime(const time_point_seconds& timeval, string time_zone) {
		_no_h24_proxy(new system_time_point(timeval), time_zone);
	}

	sys_days DateTime::GetDays() const {
		return floor<days>(*_time_point);
	}

	date_type DateTime::GetDate() const {
		return date_type{ floor<days>(GetZonedTime().get_local_time()) };
	}
	...
	hour DateTime::GetHour() const {
		return hour(GetTime().hours().count(), _h24);
	}

	minute DateTime::GetMinute() const {
		return minute(GetTime().minutes().count());
	}

	second DateTime::GetSecond() const {
		return second(GetTime().seconds().count());
	}

	zoned_time<system_duration> DateTime::GetZonedTime() const {
		return make_zoned(_tz, *_time_point);
	}

	bool DateTime::is_24h() {
		return _h24;
	}

	bool DateTime::toggle_24h() {
		bool tmp = _h24;
		_h24 = !_h24;
		return tmp;
	}

	void DateTime::make_24h(bool new_24h) {
		_h24 = new_24h;
	}

	DateTime DateTime::operator + (const system_duration& dur) const {
	...
	system_duration DateTime::operator - (const system_time_point& other) const {
		return *_time_point - other;
	}

	system_duration DateTime::operator - (const DateTime& other) const {
		return *_time_point - *(other._time_point);
	}

	DateTime& DateTime::operator += (const system_duration& dur) {
		*_time_point += dur;
		return *this;
	}
	...
	bool DateTime::operator < (const system_time_point& other) const {
		return *_time_point < other;
	}
	...
	DateTime::operator system_time_point() const {
		static system_time_point* tmp = new system_time_point(*_time_point);
		return *tmp;
	}

	DateTime::operator time_point_seconds() const {
		return floor<seconds>(*_time_point);
	}
	...
	void DateTime::set_timezone(string new_tz) {
		tie(_tz_name, _tz) = get_zone(new_tz);
	}

	string DateTime::get_timezone() const {
		return _tz_name;
	}

	string DateTime::to_string() const {
		if (_h24) {
			return format(h24format, floor<seconds>(GetZonedTime().get_local_time()));
		} else {
			return format(h12format, floor<seconds>(GetZonedTime().get_local_time()));
		}
	}

	string DateTime::get_offset_from(const DateTime& other) const {
		string out {""};
		...
		days daysoff = floor<days>(offset);
		offset -= daysoff;
		if (daysoff.count() > 0) {
		...
		hours hoursoff = floor<hours>(offset);
		offset -= hoursoff;
		minutes minutesoff = floor<minutes>(offset);
		offset -= minutesoff;
		seconds secondsoff = floor<seconds>(offset);
